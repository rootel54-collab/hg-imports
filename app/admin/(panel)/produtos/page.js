...
export default function Produtos(){
 ...
 const addCat=async()=>{if(!nc.trim())return;const {error}=await s.from('categories').insert({name:nc.trim(),slug:slug(nc)});if(error)return alert(error.message);setNc('');load()}
 const delCat=async c=>{if(!confirm(`Excluir a categoria "${c.name}"? Os produtos dela ficam sem categoria.`))return;const {error}=await s.from('categories').delete().eq('id',c.id);if(error)return alert(error.message);load()}
 ...
 if(f)return <form onSubmit={save}><button type="button" className="btn out" onClick={()=>setF(null)}>← Voltar</button><h2 style={{margin:'14px 0'}}>{f.id?'Editar':'Novo'} produto</h2>
  <label>Nome</label><input value={f.name} onChange={e=>set('name',e.target.value)} required/>
  <label>Descrição</label><textarea rows={4} value={f.description||''} onChange={e=>set('description',e.target.value)}/>
 ...
 return <><div className="sh"><h2>Produtos</h2><button className="btn" onClick={()=>{setM('');setF(E)}}>+ NOVO</button></div>
  <div className="box"><b>Categorias</b>
   <div className="row" style={{marginTop:8}}><input value={nc} onChange={e=>setNc(e.target.value)} placeholder="Ex.: Camisetas"/><button className="btn out" style={{flex:'none'}} onClick={addCat}>Adicionar</button></div>
   <div style={{display:'flex',flexWrap:'wrap',gap:8,marginTop:12}}>{cats.map(c=><span key={c.id} className="chip" style={{display:'inline-flex',alignItems:'center',gap:8,padding:'6px 12px',minWidth:0}}>{c.name}<button type="button" aria-label={`Excluir ${c.name}`} onClick={()=>delCat(c)} style={{background:'none',border:0,color:'#e11d48',fontSize:18,cursor:'pointer'}}>×</button></span>)}</div></div>
