...
export default function Cfg(){
 ...
 return <><h2 style={{marginBottom:14}}>Configurações</h2>
  <div className="box"><h3>Identidade da loja</h3><label>Logo atual</label><img src={v.logo_url||'/logo.jpg'} alt="Logo" style={{height:110,width:'auto',borderRadius:12}}/>
   ...
   <label>Imagem do banner principal</label>{v.hero_image&&<img src={v.hero_image} alt="Banner" style={{width:'100%',maxWidth:320,borderRadius:12,marginBottom:8}}/>}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>img('hero_image',e)}/>{m&&<p style={{marginTop:8}}>{m}</p>}<button type="button" className="btn out" style={{marginTop:10}} onClick={()=>{const n={...v,hero_image:''};setV(n);save(n)}}>Remover banner</button></div>
